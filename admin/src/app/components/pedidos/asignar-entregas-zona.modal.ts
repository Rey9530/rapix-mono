...
const ID_FUENTE_ZONA = "fuente-zona-entrega";
const ID_CAPA_RELLENO = "capa-relleno-zona-entrega";
const ID_CAPA_BORDE = "capa-borde-zona-entrega";

@Component({
  selector: "app-asignar-entregas-zona-modal",
  standalone: true,
  imports: [CommonModule, FormsModule],
  templateUrl: "./asignar-entregas-zona.modal.html",
  styleUrl: "./asignar-entregas-zona.modal.scss",
})
export class AsignarEntregasZonaModal
  implements OnInit, AfterViewInit, OnDestroy
{
  ...
  private dibujarPoligono(zona: Zona): void {
    ...
    this.mapa.addLayer({
      ...
      paint: { "fill-color": "#6f42c1", "fill-opacity": 0.15 },
    });
    this.mapa.addLayer({
      id: ID_CAPA_BORDE,
      type: "line",
      source: ID_FUENTE_ZONA,
      paint: { "line-color": "#6f42c1", "line-width": 2 },
    });
  }
  ...
  private dibujarMarcadores(): void {
    ...
    for (const p of this.pedidos()) {
      if (p.latitudDestino == null || p.longitudDestino == null) continue;
      const el = document.createElement("div");
      el.className = "marcador-pedido-entrega";
      ...
      const m = new mapboxgl.Marker({ element: el })
        .setLngLat([p.longitudDestino, p.latitudDestino])
        .addTo(this.mapa);
  ...
  private ajustarVista(zona: Zona): void {
    if (!this.mapa) return;
    const puntos = zona.poligono ?? [];
    const pedidos = this.pedidos().filter(
      (p) => p.latitudDestino != null && p.longitudDestino != null,
    );
  ...
  asignarTodosAlUnicoRider(): void {
    ...
    if (ids.length === 0) {
      this.toast.info("No hay pedidos en el punto de intercambio para la zona");
      return;
    }
  ...
  private ejecutarAsignacion(
    ...
    this.pedidosServicio
      .asignarMultipleEntrega({ pedidoIds, repartidorEntregaId })
      .subscribe({
        next: (r) => {
          this.enviando.set(false);
          this.asignacionRealizada = true;
          if (r.fallidos.length === 0) {
            this.toast.success(`Asignados ${r.asignados} pedidos a entrega`);
          } else {
            this.toast.warning(
